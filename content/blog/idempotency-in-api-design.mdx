...
1. Duplicate side effects such as double charges, duplicate orders, or repeated short-link creation.
2. Confusing user experience when one action appears to happen twice.
3. Extra support and reconciliation work for the team.
...
1. Idempotency increases implementation complexity in service and storage layers.
2. Input normalization rules must be stable, documented, and tested.
3. DB constraints are required to complete safety under concurrency.
4. Teams must align on response semantics so clients can rely on them.
